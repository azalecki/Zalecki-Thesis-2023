...
packages <- c( 
  ...
  "kableExtra", "broom", "usethis", "deldir", "spatstat", "webshot", "rstatix"
)
...
as_tab <- as_acs %>% 
  dplyr::select(GEOID,child, teen, yad, mad, sen, B01001_002E, B01001_026E)

as_tab$t_male <- c(as_tab$B01001_002E)
as_tab$t_fem <- c(as_tab$B01001_026E)
...
tm_shape(catchment) +
  tm_polygons(col="for_app", style="jenks", title="Foreign Born Pop", lwd= NA, border.col="lightgrey") +
...
tm_shape(web.new) +
  tm_polygons("STROOMS",palette = c("No" = "#F2D472", "Yes" = "#8C9AF5"), title="Study Rooms", lwd= NA, border.col="lightgrey") +
  tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)
...
tm_shape(web.new) +
  tm_polygons(col="VIST", palette = brewer.pal(n = 5, name = "YlGn"), style="jenks", title="Visitation", lwd= NA, border.col="lightgrey") +
  tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)
