...
as_tab <- as_acs %>% 
  dplyr::select(GEOID, child, teen, yad, mad, sen, t_male,t_fem)
...
# make tibble 
web_vor <- as_tibble(web_vor)

# make a function for factoring true and false

boolean <- function(){
  recode(is.numeric(), "0" = "Yes",  "1" = "No")
}

web.new <- mutate_at(web, vars(HSPOT, CHBOOK, UMEDIA, MHSS, CITCORNER), boolean)
             
             
```

```{r mapping-web}
tm_hotspots <-tmap_mode("plot")
tm_shape(web.new) +
  tm_polygons(col="MHSS", title="Mental Health", lwd= NA, border.col="lightgrey") +
  tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)
...
```{r outliers}
web %>% identify_outliers(COMPSES)%>%
  print(width=Inf)

no_outliers <- web %>% filter(web$CIRC < 273406)
```
...
tm_compses <- tm_shape(web_vor_chi) +
              tm_polygons(col="COMPSES", style="jenks", title="Computer Sessions", lwd= NA, border.col="lightgrey") +
              tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)
tm_compses
