...
acs_table_wt <- temp %>% merge(acs_table, by.x="tract", by.y="GEOID", all.x=T) %>% 
  mutate(latinx_app=pop_wt*latinx)

```
## Weight the social factors 

## Join block table to shapefile 
```{r join-block-tbl-shp}

blocks_wt <- inner_join(chi_blocks_shp, acs_table_wt, by= c("GEOID" = "GEOID.y"))
...
centroids <- st_centroid(blocks_wt)

```


## spatial join to library service areas 
```{r join-centroids}
# Spatial join centroids to Thiessen polygons

lib_service <- st_join(chi_vor, centroids, join= st_intersects, left=TRUE)

# Create aggregated columns of socdem variables 

agg_black <- aggregate(black ~ NAME, data = lib_service, sum)
agg_pop <- aggregate(block_pop ~ NAME, data = lib_service, sum)
agg_emp <- aggregate(emp1 ~ NAME, data = lib_service, sum)
agg_for <- aggregate(for1 ~ NAME, data= lib_service, sum)
agg_lang <- aggregate(lang2 ~ NAME, data= lib_service, sum)

# Join them together into the Vor shapefile
agg_lib <- left_join(chi_vor, agg_black, by="NAME")%>%
  rename(sumBlack = black) 

agg_lib <- left_join(agg_lib, agg_pop, by="NAME")
agg_lib <-left_join(agg_lib, agg_emp, by= "NAME")
agg_lib <-left_join(agg_lib, agg_for, by= "NAME")
agg_lib <-left_join(agg_lib, agg_lang, by= "NAME")
...
tmap_mode("plot")
tm_shape(agg_lib) +
  tm_polygons(col="sumBlack", style="jenks", title="Black Pop.", lwd= NA, border.col="lightgrey") +
tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)

tmap_mode("plot")
tm_shape(agg_lib) +
  tm_polygons(col="block_pop", style="jenks", title="Block Pop.", lwd= NA, border.col="lightgrey") +
tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)

tmap_mode("plot")
tm_shape(agg_lib) +
  tm_polygons(col="emp1", style="jenks", title="Employed", lwd= NA, border.col="lightgrey") +
tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)

tmap_mode("plot")
tm_shape(agg_lib) +
  tm_polygons(col="for1", style="jenks", title="Foreign", lwd= NA, border.col="lightgrey") +
tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)

tmap_mode("plot")
tm_shape(agg_lib) +
  tm_polygons(col="lang2", style="jenks", title="Speak Spanish", lwd= NA, border.col="lightgrey") +
tm_layout(legend.width=3, legend.text.size = .6, legend.title.size = .8, asp=0.8)
