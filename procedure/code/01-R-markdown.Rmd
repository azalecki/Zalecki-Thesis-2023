...
packages <- c( 
  ...
  "rgdal","rstudioapi", "here", "s2", "pastecs", "tmap", "knitr", "arc2r",
...
# Specify desired extent percentages
x_extent_pct <- 0.2
y_extent_pct <- 0.2
...
bbox_adjusted <- c(
  xmin = bbox$xmin - x_extent_pct * diff(c(bbox$xmin, bbox$xmax)),
  xmax = bbox$xmax + x_extent_pct * diff(c(bbox$xmin, bbox$xmax)),
  ymin = bbox$ymin - y_extent_pct * diff(c(bbox$ymin, bbox$ymax)),
  ymax = bbox$ymax + y_extent_pct * diff(c(bbox$ymin, bbox$ymax))
)

# Update the bounding box of the sf object
st_bbox(sf_points) <- bbox_adjusted
...
leaflet() %>% 
  ...
  addCircles(data = vor_clp, weight = 3, opacity= 1, color = "orange") 
```
