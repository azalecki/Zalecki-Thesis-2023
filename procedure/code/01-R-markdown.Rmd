...
packages <- c( 
  ...
  "kableExtra", "broom", "leaflet", "usethis", "deldir"
)
...
voronoi <- st_as_sf(multipoint)%>%
  st_voronoi() %>%
  st_set_crs(st_crs("+proj=utm +zone=16 +datum=WGS84"))
  
st_geometry_type(voronoi)
str(voronoi)
class(voronoi)
plot(voronoi, col = "lightgray", main = "Voronoi Diagram")

voronoi_poly = st_cast(voronoi, "MULTIPOLYGON")
class(voronoi_poly)
st_geometry_type(voronoi_poly)

# Plot the Voronoi diagram
#leaflet() %>% 
  #addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
  #addPolygons(data = voronoi, color = "lightgray", weight = 1, fillColor= "white", fillOpacity = 1)
```
